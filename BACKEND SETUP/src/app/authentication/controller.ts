...
import generateJwtFn from "../../utils/generateJwt/generateJwtFn";
...
//----------------------------------------------------------- USER REGISTRATION WITH NO EMAIL VERIFICATION --------------------------------------------------------------------------------------
  export async function userRegisterFnNoEmailVerification(req: Request, res: Response): Promise<Response> {
  ...
  res.cookie('uId',{
    ...
    secure: process.env.NODE_ENV === 'production',  // HTTPS in prod
    sameSite: 'lax',  // 'lax' allows normal navigation and is less strict
    path: '/',
  })
    return ResponseHandler(res, 201, true, finalObj, 'User registered successfully.');
...
export async function userLoginFn(req: Request, res: Response): Promise<Response> {
    ...
    if (!user) {
      return ResponseHandler(res, 200, false, null, 'Invalid email or password.');
    }
    ...
    //   );
      ...
      logger.debug('requrest reached here');
  ...
  res.cookie('uId',user._id.toString(),{
    ...
    sameSite: 'none',     // 'lax' allows normal navigation and is less strict
    path: '/',
  });
    ...
    await LoginHistoryModel.create({
       ...
       loginTime:new Date(),
       ...(geoData?.success && {geoData:geoData?.data}), 
    });
    ...
    return ResponseHandler(res, 200, true, finalObj, 'Login successful.');
  }

//----------------------------------------------------------- USER LOGOUT FN --------------------------------------------------------------------------------------
export async function USER_LOGOUT_FN (req:Request,res:Response):Promise<Response> {
...
export async function isFrontendRouteProtected(req: Request, res: Response):Promise<any> {
  try {
    ...
    return res.status(200).json({ success:true, authenticated: true, user: decoded });
  } catch (err) {
    res.clearCookie("token", {
             httpOnly: true,
             secure: process.env.NODE_ENV === "production",
    ...
    return res.status(200).json({sucess:false, authenticated: false });
  }
}
