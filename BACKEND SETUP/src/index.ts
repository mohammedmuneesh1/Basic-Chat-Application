...
import {router as ChatRoute} from './app/chat/route'
import { router as Media } from "./app/media/router";
import {router as User} from './app/user/router';
import {router as Message} from './app/message/route';
import speedLimiter from "./middleware/builtInMiddleware/slowDownLimiter";
...
app.use(speedLimiter)                       // 8) Progressive slowdown: Adds delay to requests after threshold
...
startServer();
 ...
 //------------------------------------------- SERVER WORKING SETUP START   ------------------------------------------- 
 
//  Your Express + HTTP + Socket.IO + MongoDB setup runs in this sequence:
...
// 3) Apply middleware
// 4)Register routes
// 5)Add error handlers (404 + global)
// 6)Create HTTP server → const server = http.createServer(app)
// 7)Attach Socket.IO → setupSocket(server) ← ✅ This is where it belongs
// 8) Define graceful shutdown
// 9)Start database connection
// 10)Start listening → server.listen(PORT)
 
 
 //------------------------------------------- SERVER WORKING SETUP END   ------------------------------------------- 